...
class Donation extends Model
{
    ...
    public function getAllRowsByUser($id){
        $transaction = DB::table('donation')
            ...
            ->select(DB::raw('(DATE_FORMAT(transaction.created, "%d-%m-%Y")) as transaction_created'), 'transaction_detail.detail as transaction_detail', 'donation.*', 'transaction.*', 'transaction_status.detail as transaction_status', 'scholarship.*')
    ...
    public function getAllRows(){
        ...
        $transaction = DB::table('donation')
            ...
            ->join('transaction_detail', 'transaction_detail.transaction_detail_id', '=', 'transaction.transaction_detail_id')
            ->join('transaction_status', 'transaction_status.transaction_status_id', '=', 'transaction.transaction_status_id')
            ->select(DB::raw('(DATE_FORMAT(transaction.created, "%d-%m-%Y")) as transaction_created'), 'transaction.member_id as transaction_member_id ', 'transaction_detail.detail as transaction_detail', 'donation.*', 'transaction.*', 'transaction_status.detail as transaction_status', 'scholarship.*')
            ->where('transaction.transaction_detail_id', '=', '0')
